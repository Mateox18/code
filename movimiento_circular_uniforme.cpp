...
using namespace std;
int main(){
float velT,aceT,T,D,radio,ang,rapA,per,Dt,x0,x;
float rad = 57.29;
    ...
    cin>>x0;
       cout<<"Le solicitaron una posicione especifica?"<<endl;
    cin>>x;
    if (velT && aceT && T && radio && ang && rapA && per && Dt && x && x0 <= 1)  
    // Verificamos que se hayan introducido los datos de manera correcta con el primer if
    {
        if(velT && aceT && T && radio && ang && rapA && per && Dt && x && x0 == 1){
           cout<<"Error: Ya conoce todos los datos por lo tanto el problema ya esta resuelto."<<endl;
            ...
            if(velT && rapA && aceT == 0){ // Si la velocidad y la rapidez angular se desconocen entonces se resuelve de la siguiente manera
